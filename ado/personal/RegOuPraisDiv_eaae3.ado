...
/*correction test en paramètre pr savoir si on fait test éga des betas PL, FP, HD*/
/*correction si pb reg basique, mise à vide de TabTest (pas de test d'éga des para possibles) 23/11/05*/
/*rajout d'un paramètre test(string) pr dire si oui ou non on veut test d'egalite des betas*/
/*pr les betas PL ECO, INTERM, HAUTGAMME...+ de tests d'éga des betas!!! (25/11/05)*/
...
if e(N)!=. & e(N)>=20 {     /*  <------   Test si PB dans la regression  */
    ...
    if `bg' <=0.05 {      /*cas de rejet de H0 (H0 c'est indép des résidus)*//*rajout moi...DW trop chiant à gérer*/
     ...
     if `test'==1   {    /*cas où les var expli sont pdm HD, MDD, PP (avec index ou non)*/
