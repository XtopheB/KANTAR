...
/*28/09/06 on adapte MN2 pour que la procédure soit valable sur les données 98-01 mais aussi à partir de 2002*/
/*enwp-->cvwpenwp, 
Leader 5135 avant, 104 après
liste des magasins HD qui change*/
/*21/04/08 correction moda de Leader c'est 52 (pas 104 puisqu'il faut prendre moda de sa3 d'origine à cause du rename!!!)*/
/*23/07/08 Netto rajoutée à la liste des HD pour nouveaux fichiers (CDM devient Netto en 2001!)*/
/*Version 2.0   8/06/2010   Variables créées seront missing par défaut (au lieu de 0)*/
/*                          Numéro de version créé (via note)                        */
/*Version 2.1  12/08/10  correction erreur sur HD (hardis erroné) + mhd initialisé à 0 (C et V)*/
/*Version 2.2  24/08/10  marque Leader se retrouve ds plusieurs modalités --> liste plutôt que variable locale (V)*/
/*Version 2.3  2/09/11  : Changemment de nom du programme MN2G  inspiré de MN2NF  */
/*                       : Changement de nom pour pu qui devient Pu (majuscule)  enonformtié avec DataImportG.do              */ 
/* Version 2.4 01/08/2012 : labelisation variables                        */
/*--------------*/
* UTILISATION : 
/*--------------*/

/* 
- MN2NF est une procédure permettant de créer la variable de classification des marques : "brand2" (MN, MDD, HD, PP, MR)
- elle est basée sur des hypothèses décidées par un groupe de travail de l'équipe MAIA
- les hypothèses sont décrites en annexes du document "ToutSurSecodip.pdf"
- MN2NF est une procédure à lancer sur un fichier d'achats d'un produit donné (1 seul produit!) sur une ou plusieurs années
- Les variables suivantes devront être présentes dans le fichier : 
    * la variable enseigne devra être présente (ctwpenwp ou enwp selon année d'étude)
    * la variable sa2 devra être présente ET labellisée (attention, sa2 = range, ne pas confondre avec sa3 = range.appellation)
    * la variable sa7 devra être présente (et ses modalités devront être 0 ou 1...attention, pas 1 ou 2...)
    * Remarque : la variable sa7 aura pu être vérifiée (si fichier vérifié via VerifNF.do, via procédure Distinct_marque.ado, 
                                                        correction des ambiguités sur une marque)
    * la variable Pu (prix unitaire) devra être présente (en majuscule (Version 2/09/2011)  
    * la variable noreg devra être présente
 ------------------------------------------------------------------------------------------------------*/
set output error
set more off
...
local NumVersion "2.4"

/***********************************************************************************************/
/****   ETAPE POUR AFFECTER LES BONNES VARIABLES ET CODAGES SELON LA FORMULE DES DONNEES  ****/
capture confirm new variable ctwpenwp   /*variable enseigne pour données APRES 2002*/
if _rc==0{  /*ancienne formule (données 1998-2001)*/
...
else {  /*nouvelle formule (données 2001NF, 2002, ..., 2007)*/
    local listhardis "11 72 132 143 155 144 229 241  244 251 285 295 570 330 835"   /*835 Netto (anciennement CDM 72) rajoutée 23/07/08*/  
...
/******************************************************************/
capture drop hardis  
gen hardis=0        /*12/08/10 V : boulette détectée avec Fabian car sinon test_hardis basé uniquement sur les enseignes HD (car moy sur hardis; et "." non pris en cpte) et affecte 1!!! */            
...
/*24/08/10 écrire la condition que la marque est différente de Leader (via boucle car plusieurs moda pour la marque Leader Price)*/
...
preserve
    set more off
    gen mdd=(sa7==1 & mhd==0) if  (sa7!=. & mhd!=.)  /*On fait confiance a la variable sa7*/ /*New 8/06/10 missing par défaut*/
...
gen pp=(sans_marque==1 & prix_moyen_par_marque<=borne_pour_sans_marques)   /*Les marques "sans marque" sont classsees pp si leur prix moyen est inférieur ou egal au prix moyen des marques de hard-discount plus deux fois l'écart type.*/
bysort pp : sum Pu
...
/* labelisation  */
capture label var hardis "Indicatrice Enseigne HD"
capture label var mhd "Indicatrice HD"
capture label var pp "Indicatrice Premier Prix"
capture label var MN "Indicatrice Marque Nationale"
capture label var MR "Indicatrice Marque Regionale"
capture label var mdd "Indicatrice MDD"

capture label var brand2 "Typologie des Marques"
